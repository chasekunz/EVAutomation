...
//     int smoothLen = 23;
  ...
  switch (lineType)
  {
    case LINE_HORIZONTAL:
    {
      ...
      printf("In the vertical line section\n");
    }
    break;
    ...
    case LINE_VERTICAL:
    {
      ...
      FLOAT_MAT_ELEM_TYPE derivp[] = //{1.000000e-11, 8.800000e-10, 3.531000e-08, 8.536000e-07, 1.383415e-05, 1.581862e-04, 1.306992e-03, 7.852691e-03, 3.402475e-02, 1.038205e-01, 2.137474e-01, 2.781496e-01, 2.137474e-01, 1.038205e-01, 3.402475e-02, 7.852691e-03, 1.306992e-03, 1.581862e-04, 1.383415e-05, 8.536000e-07, 3.531000e-08, 8.800000e-10, 1.000000e-11};
        ...
        //{-2.328306e-10, -6.984919e-09, -1.008157e-07, -9.313226e-07, -6.178394e-06, -3.129616e-05, -1.255888e-04, -4.085824e-04, -1.092623e-03, -2.416329e-03, -4.408169e-03, -6.530620e-03, -7.510213e-03, -5.777087e-03, -5.777087e-04, 6.932504e-03, 1.372058e-02, 1.646470e-02, 1.372058e-02, 6.932504e-03, -5.777087e-04, -5.777087e-03, -7.510213e-03, -6.530620e-03, -4.408169e-03, -2.416329e-03, -1.092623e-03, -4.085824e-04, -1.255888e-04, -3.129616e-05, -6.178394e-06, -9.313226e-07, -1.008157e-07, -6.984919e-09, -2.328306e-10};
...
void mcvGetLanes(const CvMat *inImage, const CvMat* clrImage,
                 ...
                 float& lateralError,float& yawError, LineState* state)
{
  //input size
  CvSize inSize = cvSize(inImage->width, inImage->height);
...
#warning "clean negative parts in filtered image"
  ...
  // 	SHOW_RECT(mask, "Mask");
	...
	//debug
    if(DEBUG_LINES || DEBUG_IPM) {//#ifdef DEBUG_GET_STOP_LINES
  ...
  mcvGetControlOutput(fipm, ipm, stopLineConf,
                    state,lateralError, yawError);
  ...
  if(DEBUG_LINES || DEBUG_IPM) {//#ifdef DEBUG_GET_STOP_LINES
    ...
    SHOW_IMAGE(imageClr, "Detected lanes", 10);
    //cvReleaseMat(&image);
    cvReleaseMat(&dbIpmImage);
    //cvReleaseMat(&dbIpmImageThresholded);
...
*
*/
void mcvGetControlOutput(const CvMat* rawipm,const CvMat* fipm,
                        LaneDetectorConf *lineConf, LineState *state,
                        float& lateralError,float& yawError){
    // get ipmSplines saved in state
    vector<Spline> splines = state->ipmSplines;
  ...
  if (lineConf->ransacSpline)
  {
    ...
    for(int i=0; i<(int)splines.size(); i++)
    {
          ...
          || splineStatus & HorizontalSpline))
      {
        ...
        //cv::Mat points_p = cv::cvarrToMat(points);
        //std::cout << "Points:" << points_p << endl;

        //cv::Mat p_p = cv::cvarrToMat(p);
        //std::cout << "p:" << p_p << endl;
...
#warning "Check this later: extension in IPM. Check threshold value"
        ...
        //printf("Spline score: %f\t threshold: %f\n",splineScores[i],lineConf->finalSplineScoreThreshold);
...
/*
    ...
    for(int i=0; i<(int)splines.size(); i++)
    {
      ...
      mcvCheckSpline(splines[i],
	    ...
	    //********** I took this whole if statement out, bad transforms ck ******************
	    ...
	    // ********* I took this if structure out, it was taking away good lines ck *******
        ...
        //extend spline on regular image
        CvMat* p = mcvExtendPoints(image, points,
                                   lineConf->extendAngleThreshold,
                                   lineConf->extendMeanDirAngleThresholdBack,
                                   lineConf->extendMeanDirAngleThresholdForward,
                    ...
                    cvRect(0, lineConf->extendRectTop,
                           ...
                           lineConf->extendRectBottom -lineConf->extendRectTop -1)); // I changed this to cover the whole bottom of the image -ck
    ...
    LaneDetectorConf *stopLineConf)
{
  ...
  stopLineConf->extendMeanDirAngleThresholdBack = stopLineParserInfo.extendMeanDirAngleThresholdBack_arg;
  stopLineConf->extendMeanDirAngleThresholdForward = stopLineParserInfo.extendMeanDirAngleThresholdForward_arg;
                      ...
                      float *lineAbc)
{
  ...
  // With only a single point cvSCD crashes
  // This creates two points, the orignal first and a second offset by an arbitrary 0.0001
  if(points->cols == 1){
    printf("*************** There is only one point here ***************\n");
    cpoints = cvCreateMat(points->rows, points->cols + 1, CV_32FC1);
    CV_MAT_ELEM(*cpoints, float, 0, 0) = CV_MAT_ELEM(*points, float, 0, 0);
    CV_MAT_ELEM(*cpoints, float, 1, 0) = CV_MAT_ELEM(*points, float, 1, 0);
    CV_MAT_ELEM(*cpoints, float, 0, 1) = CV_MAT_ELEM(*points, float, 0, 0) + 0.0001;
    CV_MAT_ELEM(*cpoints, float, 1, 1) = CV_MAT_ELEM(*points, float, 1, 0) + 0.0001;
  }
  else{
    cpoints = cvCloneMat(points);
  }


  //CvMat *cpoints = cvCloneMat(points);
  //SHOW_MAT(points,"points");
  //SHOW_MAT(cpoints,"cpoints");
 ...
 * \param meanDirAngleThresholdBack angle threshold from mean direction back
 * \param meanDirAngleThresholdForward angle threshold from mean direction forward
...
CvMat*  mcvExtendPoints(const CvMat *im, const CvMat *inPoints,
                        float angleThreshold, float meanDirAngleThresholdBack,
                        float meanDirAngleThresholdForward, int linePixelsTangent, int linePixelsNormal,
                        ...
                        CvRect bbox, bool smoothPeaks)
{
  ...
  //cv::Mat test = cv::cvarrToMat(inPoints);
  ...
  while(cont)
  {
    ...
    if (mcvIsPointInside(nextPoint, bbox))
    {
        ...
        //mcvDrawLine(imageClr,line,CV_RGB(0,1,0),1);
        //SHOW_IMAGE(imageClr, str, 10);
      ...
      //    !mcvIsValidPeak(peak, pprevTangent, curPoint, angleThreshold) ||
        bool isValidTangentFromLastPoint =  mcvIsValidPeak(peak, tangent, curPoint, angleThreshold);
        bool isValidPeakFromMeanDir = mcvIsValidPeak(peak, meanDir, curPoint, meanDirAngleThresholdBack);


	    if (!isValidTangentFromLastPoint || !isValidPeakFromMeanDir)
	    {
        peak = nextPoint;
        deviationCount++;
	    }
	    ...
	    if (DEBUG_LINES){
                    // tell why deviation
            if (!isValidTangentFromLastPoint)
                printf("Deviation: tangent from last point too large\n");
            if (!isValidPeakFromMeanDir)
                printf("Deviation: difference from direction mean too large\n");
    ...
    //show image
    cvRectangle(imageClr,cvPoint(bbox.x,bbox.y),cvPoint(bbox.width + bbox.x,bbox.height + bbox.y),cvScalar( 0, 255, 255 ));
    printf("BBox: %i %i %i %i\n",bbox.x,bbox.y,bbox.width,bbox.height);
    SHOW_IMAGE(imageClr, str, 10);
    //printf("Paused at line 5920\n");
    //cvWaitKey(0);
    }//#endif
  }  // while cont
...
//         !mcvIsValidPeak(peak, pprevTangent, curPoint, angleThreshold) ||
        ...
        bool isValidTangentFromLastPoint =  mcvIsValidPeak(peak, tangent, curPoint, angleThreshold);
        bool isValidPeakFromMeanDir = mcvIsValidPeak(peak, meanDir, curPoint, meanDirAngleThresholdForward);
