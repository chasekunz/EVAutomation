...
//     cvCvtColor(raw_mat, imDisplay, CV_RGB2BGR);
    ...
    if(splines.size() == 2){
        ...
        // store points for first spline
        for(int i = 0;i<points1->rows;i++){
            polyPoints[0][i] = CvPoint((int)CV_MAT_ELEM(*points1, float, i, 0), (int)CV_MAT_ELEM(*points1, float, i, 1));
        }

         //store points for second spline in reverse order
        for(int i = 0;i<points2->rows;i++){
            polyPoints[0][points1->rows + points2->rows - 1 - i] = CvPoint(CV_MAT_ELEM(*points2, float, i, 0), CV_MAT_ELEM(*points2, float, i, 1));
        }

        CvPoint* pptPoly[1] = { polyPoints[0] };
        int npt[] = { points1->rows + points2->rows};

        cvFillPoly( imDisplay, pptPoly, npt, 1, cvScalar( 255, 0, 0), 8 );
    }
    ...
    if (options.show_flag)
    {
      ...
      //printf("*****Number of Splines at Lines Detected: %i *****\n",(int)splines.size());

//      // show IPM region
//      Line line;
//      line.startPoint = cvPoint2D32f(lanesConf.ipmLeft,lanesConf.ipmBottom);
//      line.endPoint = cvPoint2D32f(lanesConf.ipmLeft,lanesConf.ipmTop);
//      mcvDrawLine(imDisplay, line, CV_RGB(255, 0, 0), 1);
//      line.startPoint = cvPoint2D32f(lanesConf.ipmLeft,lanesConf.ipmTop);
//      line.endPoint = cvPoint2D32f(lanesConf.ipmRight,lanesConf.ipmTop);
//      mcvDrawLine(imDisplay, line, CV_RGB(255, 0, 0), 1);
//      line.startPoint = cvPoint2D32f(lanesConf.ipmRight,lanesConf.ipmTop);
//      line.endPoint = cvPoint2D32f(lanesConf.ipmRight,lanesConf.ipmBottom);
//      mcvDrawLine(imDisplay, line, CV_RGB(255, 0, 0), 1);
//      line.startPoint = cvPoint2D32f(lanesConf.ipmRight,lanesConf.ipmBottom);
//      line.endPoint = cvPoint2D32f(lanesConf.ipmLeft,lanesConf.ipmBottom);
//      mcvDrawLine(imDisplay, line, CV_RGB(255, 0, 0), 1);
